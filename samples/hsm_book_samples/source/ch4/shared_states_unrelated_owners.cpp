...
struct SharedStates {
  template <typename BaseState> struct PlayAnim_Done : BaseState {};

  template <typename BaseState> struct PlayAnim : BaseState {
    using BaseState::Owner;

    virtual void OnEnter(const char *animName, bool loop = true,
                         const Transition &doneTransition =
                             SiblingTransition<PlayAnim_Done<BaseState>>()) {
